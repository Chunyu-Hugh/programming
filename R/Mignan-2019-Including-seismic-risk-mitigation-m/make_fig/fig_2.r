...
E1 <- data.frame(x = x_value, y = y_value1,z = type)
#here I need to modify some parameters to make the figure beautiful
new_E <- E1[y_value1>0,]
g1 <- ggplot(data = new_E, mapping = aes(x = x, y = y/1e6,group = z, ,color = z))+
geom_smooth()+
...
E2 <- data.frame(x = x_value, y = y_value2,z = type)
#here I need to modify some parameters to make the figure beautiful
new_E <- E2[y_value2>0,]

g2 <- ggplot(data = new_E, mapping = aes(x = x, y = y/1e6,group = z ,color = z))+
geom_smooth()+
theme(plot.title = element_text(hjust = 0.5))+
