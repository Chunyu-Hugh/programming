...
y_value <- c(E.el[1,,1],E.el[2,,1],E.el[3,,1],E.el[4,,1],E.el[5,,1],E.el[6,,1])

E <- data.frame(x = x_value, y = y_value,type = type)

#here I need to modify some parameters to make the figure beautiful
new_E <- E[y_value>0,]

ggplot(data = new_E, mapping = aes(x = x, y = y/1e6,group = type ,type = type ,color = type))+
geom_smooth()+
...
type <- rep(c(4:9),each = length(V.inj))
x_value <- rep(seq(0,0.4,0.001),times = 6)
y_value <- c(E.el[1,,2],E.el[2,,2],E.el[3,,2],E.el[4,,2],E.el[5,,2],E.el[6,,2])
