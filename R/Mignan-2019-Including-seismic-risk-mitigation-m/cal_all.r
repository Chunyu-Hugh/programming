...
fct_hazardcurve <- function(rate,intensity,sigma,nsigma,increment,distrib){
  int <- increment; nbi <- length(int)
  nbm <- length(rate)
  rate_mi <- numeric(nbm*nbi); dim(rate_mi) <- c(nbm,nbi)
  ...
  for(i in 1:nbi) rate_all[i] <- sum(rate_mi[,i])
  Pr <- 1-exp(-rate_all)
  indNA <- which(Pr == 0)
...
fct_DGD_binomial <- function(MDG){
  ...
  Pr <- numeric(6)
  for(i in 1:6) Pr[i] <- factorial(5)/(factorial(dg[i])*factorial(5-dg[i]))*((MDG/5)^dg[i])*((1-MDG/5)^(5-dg[i]))
  return(Pr)
}
...
for(i in 1:nsite) for(j in 1:nIPE) for(k in 1:nz) for(l in 1:nd){
...
nVi <- length(Vi)
MDGval <- numeric(nVi*ni); dim(MDGval) <- c(nVi,ni)
DG_pr <- numeric(nVi*ni*6); dim(DG_pr) <- c(nVi,ni,6)
Deathval <- numeric(nVi*ni); dim(Deathval) <- c(nVi,ni)

for(i in 1:nVi) for(j in 1:ni){
...
for(sim in 48:nsim){
  ...
  xi <- xmin+seq(xmax-xmin); nx <- length(xi)
  yi <- ymin+seq(ymax-ymin); ny <- length(yi)
  ...
  for(i in 2:nEGS.max){
    ...
    if(count_circle(X_EGS_tmp, siting.map, radius) == "go"){
      ...
      indrem <- c()
      for(j in 1:length(indmaxout)) indrem <- c(indrem, which(siting.map1$sector == indmaxout[j]) )
  ...
  for(i in 2:nEGS.max){
    ...
    if(count_circle(X_EGS_tmp, siting.map, radius) == "go"){
      ...
      indrem <- c()
      for(j in 1:length(indmaxout)) indrem <- c(indrem, which(siting.map2$sector == indmaxout[j]) )
      indloc <- unique(c(indloc, indrem))
    }
  }
  X_EGS2 <- X_EGS
...
for(g in 2:ngen){
  ...
  for(i in 1:NP){
    ...
    indrdm1 <- ceiling(runif(1)*NP); while(indrdm1 == i) indrdm1 <- ceiling(runif(1)*NP)
    indrdm2 <- ceiling(runif(1)*NP); while(indrdm2 == i | indrdm2 == indrdm1) indrdm1 <- ceiling(runif(1)*NP)
    indrdm3 <- ceiling(runif(1)*NP); while(indrdm3 == i | indrdm3 == indrdm1 | indrdm3 == indrdm2) indrdm1 <- ceiling(runif(1)*NP)
